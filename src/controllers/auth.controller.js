...
const setUserJWT = require("../helpers/setUserJWT");
const setGoogleUserJWT = require("../helpers/setGoogleUserJWT");
...
module.exports = {
  login: async (req, res) => {
    ...
    */
    ...
    const data = password
      ? await setUserJWT(req.body)
      : await setGoogleUserJWT(req.body);
    ...
    */
    ...
    jwt.verify(refresh, process.env.REFRESH_KEY, async (err, decoded) => {
      if (err) {
        ...
        const data = decoded?.password
          ? await setUserJWT(decoded, refresh)
          : await setGoogleUserJWT(decoded, refresh);
